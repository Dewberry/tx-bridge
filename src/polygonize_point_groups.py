...
def fn_get_hull_polygons(dict_params):
    ...
    for i in range(int_last_valid_cluster_index + 1):
        ...
        dict_bridge_hull = {"las_path": str_las_path, "geometry": shp_poly_bridge_hull}
...
def fn_polygonize_point_groups(str_las_input_directory, str_output_dir, int_class, flt_epsilon, int_min_samples):
    ...
    if len(list_files_with_points) > 0:
        ...
        list_gdf_hulls = list(
            tqdm.tqdm(
                p.imap(fn_get_hull_polygons, list_of_dict),
                total=l,
                desc="Processing LAS",
        ...
        raise RuntimeError("No bridge points here.")
...
if __name__ == "__main__":
    ...
    fn_polygonize_point_groups(str_las_input_directory, str_output_dir, int_class, flt_epsilon, int_min_samples)
